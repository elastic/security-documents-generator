...
const CASE_SUBCATEGORIES: Record<string, string[]> = {
  security_incident: ['malware_infection', 'data_exfiltration', 'unauthorized_access', 'system_compromise'],
  threat_hunting: ['apt_activity', 'suspicious_behavior', 'ioc_investigation', 'threat_actor_tracking'],
  vulnerability_investigation: ['critical_vulnerability', 'zero_day', 'patch_verification', 'exposure_assessment'],
  compliance_violation: ['pci_dss_violation', 'gdpr_breach', 'sox_compliance', 'hipaa_violation'],
  insider_threat: ['data_theft', 'policy_violation', 'privilege_abuse', 'suspicious_activity'],
  malware_analysis: ['ransomware', 'trojan', 'rootkit', 'backdoor', 'cryptominer'],
  data_breach: ['customer_data', 'financial_data', 'intellectual_property', 'personal_information'],
  network_intrusion: ['lateral_movement', 'command_control', 'persistence', 'reconnaissance'],
  phishing_investigation: ['spear_phishing', 'credential_harvesting', 'business_email_compromise', 'social_engineering'],
  fraud_detection: ['payment_fraud', 'identity_theft', 'account_takeover', 'transaction_anomaly']
};
...
function generateCaseTitle(category: string, subcategory: string, incidentType: string): string {
...
Affected systems isolated pending detailed analysis.
  ...
  const generator = descriptionTemplates[category] || descriptionTemplates.security_incident;
  return generator();
}
...
async function generateMitreData(): Promise<{
  ...
  try {
    ...
    return {
      ...
      tactic_ids: [`TA${faker.number.int({ min: 1, max: 12 }).toString().padStart(4, '0')}`],
      framework: 'MITRE ATT&CK',
...
export async function generateSecurityCase(
  ...
  const subcategory = faker.helpers.arrayElement(CASE_SUBCATEGORIES[category] || ['general']);
  const incidentType = faker.helpers.arrayElement(INCIDENT_TYPES);
  const severity = faker.helpers.arrayElement(['low', 'medium', 'high', 'critical']) as 'low' | 'medium' | 'high' | 'critical';
  const priority = faker.helpers.arrayElement(['low', 'medium', 'high', 'critical']) as 'low' | 'medium' | 'high' | 'critical';
  ...
  const description = generateCaseDescription(category, subcategory, incidentType, severity, affectedSystems);
  ...
  const tags = [
    ...
    ...faker.helpers.arrayElements(['urgent', 'escalated', 'external-threat', 'insider-threat', 'automated-detection', 'manual-investigation'], { min: 0, max: 3 })
  ];
  ...
  const baseCase: SecurityCaseData = {
    ...
    security: {
      ...
      threat_level: faker.helpers.arrayElement(['low', 'medium', 'high', 'critical']) as 'low' | 'medium' | 'high' | 'critical'
    },
    timeline: {
      discovery_time: discoveryTime.toISOString(),
      first_occurrence: firstOccurrence.toISOString(),
      last_occurrence: faker.date.between({ from: firstOccurrence, to: discoveryTime }).toISOString(),
      estimated_containment: faker.date.future({ years: 0.1 }).toISOString()
    },
    investigation: {
      lead_analyst: leadAnalyst,
      team,
      status: faker.helpers.arrayElement(['open', 'in-progress', 'closed']) as 'open' | 'in-progress' | 'closed',
      findings: faker.helpers.maybe(() => faker.helpers.arrayElements([
      ...
      ], { min: 1, max: 3 }), { probability: 0.7 }),
      recommendations: faker.helpers.maybe(() => faker.helpers.arrayElements([
    ...
    metadata: {
      ...
      estimated_effort_hours: faker.helpers.maybe(() => faker.number.int({ min: 2, max: 40 }), { probability: 0.8 })
    }
  };

  if (includeMitre) {
    baseCase.mitre = await generateMitreData();
    baseCase.tags.push('mitre-attack', ...baseCase.mitre.technique_ids.slice(0, 2));
  }
...
This case was created automatically based on alert severity and pattern matching.
  ...
  const caseData: SecurityCaseData = {
    ...
    security: {
      ...
      incident_type: incidentType,
      priority: severity,
      impact_assessment: 'Assessment pending - case created from security alert',
    ...
    metadata: {
      ...
      last_updated: now.toISOString(),
      escalation_level: severity === 'critical' ? 4 : severity === 'high' ? 3 : 2,
      alert_count: 1
    }
  };
