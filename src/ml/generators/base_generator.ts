...
export abstract class BaseMLGenerator {
  ...
  protected getFieldValue(field: string, num: number): string | number {
    ...
    if (this.options.themedData) {
      if (field.includes('user.name') && this.options.themedData.usernames) {
        return this.options.themedData.usernames[
          num % this.options.themedData.usernames.length
        ];
      }
      if (field.includes('host.name') && this.options.themedData.hostnames) {
        return this.options.themedData.hostnames[
          num % this.options.themedData.hostnames.length
        ];
      }
      if (
        field.includes('process.name') &&
        this.options.themedData.processNames
      ) {
        return this.options.themedData.processNames[
          num % this.options.themedData.processNames.length
        ];
      }
      if (
        (field.includes('domain') || field.includes('url')) &&
        this.options.themedData.domains
      ) {
        if (num === 1) {
          ...
          return this.options.themedData.domains[
            num % this.options.themedData.domains.length
          ];
        }
      }
    }
  ...
  public async generateAll(): Promise<MLDataDocument[]> {
    ...
    for (const doc of this.generate()) {
      ...
      if (documentCount >= maxDocuments) {
        console.warn(
          `Generated maximum ${maxDocuments} documents for ${this.jobId}`,
        );
        break;
      }
    }
  ...
  public getProgress(): number {
    ...
    return Math.min(
      1,
      (this.currentTime - this.startTime) / (this.endTime - this.startTime),
    );
  }
}
