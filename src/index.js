...
import { deleteAllAlerts, deleteAllEvents, deleteAllLogs, generateAlerts, generateEvents, generateGraph, generateLogs, generateCorrelatedCampaign, } from './commands/documents';
...
const parseIntBase10 = (input) => parseInt(input, 10);
...
const applyPhase3ConfigOverrides = (options) => {
    ...
    if (options.focusTactic && config.mitre) {
        ...
        config.mitre.focusTactic = options.focusTactic;
...
program
    ...
    .option('--mitre', 'use MITRE ATT&CK framework for realistic attack scenarios', false)
    .option('--sub-techniques', 'include MITRE sub-techniques in generated alerts (requires --mitre)', false)
    .option('--attack-chains', 'generate realistic attack chains with multiple techniques (requires --mitre)', false)
    .option('--large-scale', 'enable performance optimizations for large datasets (>1000)', false)
    .option('--focus-tactic <tactic>', 'focus on specific MITRE tactic (e.g., TA0001 for Initial Access)')
    .option('--start-date <date>', 'start date for data generation (e.g., "7d", "1w", "2024-01-01")')
    .option('--end-date <date>', 'end date for data generation (e.g., "now", "1d", "2024-01-10")')
    .option('--time-pattern <pattern>', 'time distribution pattern: uniform, business_hours, random, attack_simulation, weekend_heavy')
    .option('--false-positive-rate <rate>', 'percentage of alerts to mark as false positives (0.0-1.0) for testing detection rules', '0.0')
    .option('--multi-field', 'generate hundreds of additional contextual security fields (99% token reduction)', false)
    .option('--field-count <count>', 'number of additional fields to generate (requires --multi-field)', '200')
    .option('--field-categories <categories>', 'specific field categories to include (comma-separated): behavioral_analytics,threat_intelligence,performance_metrics,security_scores,audit_compliance,network_analytics,endpoint_analytics')
    .option('--field-performance-mode', 'optimize multi-field generation for speed over variety (requires --multi-field)', false)
    .description('Generate AI-powered security alerts with optional MITRE ATT&CK scenarios')
    ...
    const fieldCategories = options.fieldCategories ? options.fieldCategories.split(',').map((c) => c.trim()) : undefined;
    ...
    if (options.fieldCount && !useMultiField) {
        console.error('Error: --field-count flag requires --multi-field to be enabled');
        process.exit(1);
    }
    if (options.fieldCategories && !useMultiField) {
        console.error('Error: --field-categories flag requires --multi-field to be enabled');
        process.exit(1);
    }
    if (fieldPerformanceMode && !useMultiField) {
        console.error('Error: --field-performance-mode flag requires --multi-field to be enabled');
        process.exit(1);
    }
    if (fieldCount < 1 || fieldCount > 1000) {
        console.error('Error: --field-count must be between 1 and 1000');
        process.exit(1);
    }
    ...
    if (fieldCategories) {
        ...
        const invalidCategories = fieldCategories.filter((cat) => !validCategories.includes(cat));
        if (invalidCategories.length > 0) {
            console.error(`Error: Invalid field categories: ${invalidCategories.join(', ')}. Valid categories: ${validCategories.join(', ')}`);
            process.exit(1);
        }
    }
    ...
    if (options.focusTactic && !useMitre) {
        console.error('Error: --focus-tactic flag requires --mitre to be enabled');
        process.exit(1);
    }
    ...
    if (options.focusTactic) {
        ...
        if (!validTactics.includes(options.focusTactic)) {
            console.error(`Error: Invalid tactic ${options.focusTactic}. Valid tactics: ${validTactics.join(', ')}`);
            process.exit(1);
        }
    }
    ...
    generateAlerts(alertsCount, userCount, hostCount, space, useAI, useMitre, timestampConfig, falsePositiveRate, multiFieldConfig);
});
program
    ...
    .option('--end-date <date>', 'end date for data generation (e.g., "now", "1d", "2024-01-10")')
    .option('--time-pattern <pattern>', 'time distribution pattern: uniform, business_hours, random, attack_simulation, weekend_heavy')
    .description('Generate AI-powered security events with optional MITRE ATT&CK scenarios')
    .action((n, options) => {
    // Validate flag dependencies
...
program
    ...
    .option('--types <types>', 'log types to generate (comma-separated): system,auth,network,endpoint', 'system,auth,network,endpoint')
    .option('--claude', 'use Claude AI instead of OpenAI', false)
    .option('--start-date <date>', 'start date for data generation (e.g., "7d", "1w", "2024-01-01")')
    .option('--end-date <date>', 'end date for data generation (e.g., "now", "1d", "2024-01-10")')
    .option('--time-pattern <pattern>', 'time distribution pattern: uniform, business_hours, random, attack_simulation, weekend_heavy')
    ...
    .option('--field-categories <categories>', 'specific field categories to include (comma-separated): behavioral_analytics,threat_intelligence,performance_metrics,security_scores,audit_compliance,network_analytics,endpoint_analytics')
    .option('--field-performance-mode', 'optimize multi-field generation for speed over variety (requires --multi-field)', false)
    .option('--session-view', 'generate Session View compatible data with process hierarchies and terminal output', false)
    .option('--visual-analyzer', 'generate Visual Event Analyzer compatible data with process entity tracking', false)
    .action(async (options) => {
    ...
    const logTypes = options.types.split(',').map((t) => t.trim());
    const useMultiField = options.multiField || false;
    const fieldCount = parseInt(options.fieldCount || '200');
    const fieldCategories = options.fieldCategories ? options.fieldCategories.split(',').map((c) => c.trim()) : undefined;
    const fieldPerformanceMode = options.fieldPerformanceMode || false;
    ...
    const invalidTypes = logTypes.filter((type) => !validTypes.includes(type));
    if (invalidTypes.length > 0) {
        console.error(`Error: Invalid log types: ${invalidTypes.join(', ')}. Valid types: ${validTypes.join(', ')}`);
        process.exit(1);
    }
    // Validate multi-field options
    if (options.fieldCount && !useMultiField) {
    ...
    if (sessionView || visualAnalyzer) {
        ...
        if (sessionView)
            console.log(`  📱 Session View: Process hierarchies and terminal output`);
        if (visualAnalyzer)
    ...
    await generateLogs(logCount, hostCount, userCount, useAI, logTypes, timestampConfig, multiFieldConfig, sessionView, visualAnalyzer);
});
program
...
program
    ...
    .description('Generate AI-powered entity relationship graph with realistic alerts')
...
program
    ...
    .option('-s, --space <space>', 'Space to delete alerts from (default: all spaces)')
    .action(async (options) => {
    try {
...
program
    ...
    .option('-s, --space <space>', 'Space to delete events from (default: all spaces)')
    .action(async (options) => {
    try {
...
program
    ...
    .option('--types <types>', 'log types to delete (comma-separated): system,auth,network,endpoint', 'system,auth,network,endpoint')
    .action(async (options) => {
    try {
        const logTypes = options.types.split(',').map((t) => t.trim());
        // Validate log types
...
program
    ...
    console.log(`Testing MITRE AI integration with ${alertCount} alerts in space '${space}'...`);
    // Apply Phase 3 configuration overrides if flags are used
    if (options.claude) {
        applyPhase3ConfigOverrides(options);
    }
...
program
    ...
    .description('Generate sophisticated multi-stage attack campaigns (Phase 3 Implementation)')
    .argument('<type>', 'Campaign type: apt, ransomware, insider, supply-chain, scale-test')
    .option('-c, --complexity <level>', 'Campaign complexity (low|medium|high|expert)', 'high')
    ...
    .option('--large-scale', 'enable performance optimizations for large datasets', false)
    .option('--realistic', '🔗 Generate realistic source logs that trigger alerts (Phase 2 Integration)', false)
    .option('--logs-per-stage <count>', 'number of logs to generate per attack stage (realistic mode)', '8')
    .option('--detection-rate <rate>', 'detection rate (0.0-1.0) - what percentage of activity gets detected', '0.4')
    .option('--start-date <date>', 'start date for data generation (e.g., "7d", "1w", "2024-01-01")')
    .option('--end-date <date>', 'end date for data generation (e.g., "now", "1d", "2024-01-10")')
    .option('--time-pattern <pattern>', 'time distribution pattern: uniform, business_hours, random, attack_simulation, weekend_heavy')
    ...
    if (campaignType === 'scale-test') {
        ...
        console.log(`📝 Generating sophisticated ${campaignType} campaign with ${eventCount} events...`);
        ...
        try {
            ...
            if (options.realistic) {
                ...
                const realisticConfig = {
                    ...
                    timestampConfig: {
                        ...
                        pattern: (options.timePattern || 'attack_simulation')
                    }
                };
            ...
            const sophisticatedGeneration = async () => {
                ...
                const simulation = await simulationEngine.generateAttackSimulation(campaignType, options.complexity);
                ...
                console.log(`  📅 Duration: ${simulation.campaign.duration.start.toISOString().split('T')[0]} → ${simulation.campaign.duration.end.toISOString().split('T')[0]}`);
                ...
                const correlatedEvents = await simulationEngine.generateCampaignEvents(simulation, targetCount, eventCount, options.space, useMitre, timestampConfig);
            ...
            console.log(`⏱️  Timeout set to ${Math.round(timeoutMs / 1000)} seconds for ${eventCount} events`);
            const result = await Promise.race([
                sophisticatedGeneration(),
                new Promise((_, reject) => setTimeout(() => reject(new Error(`Sophisticated generation timeout after ${Math.round(timeoutMs / 1000)}s`)), timeoutMs)),
            ]);
            ...
            for (const alert of result) {
                bulkOps.push({ index: { _index: alertIndex, _id: alert['kibana.alert.uuid'] } }, { ...alert });
            }
            ...
            const actualHostCount = Math.min(targetCount, Math.ceil(eventCount * 0.6));
            const actualUserCount = Math.min(Math.ceil(eventCount * 0.4), actualHostCount - 1);
            await generateAlerts(eventCount, actualHostCount, actualUserCount, options.space, useAI, useMitre, timestampConfig);
        }
        ...
        console.log(`📊 Generated ${eventCount} AI-powered events in ${options.space} space`);
        console.log(`🧠 AI Provider: ${useClaude ? 'Claude' : 'OpenAI'}`);
        if (useMitre) {
