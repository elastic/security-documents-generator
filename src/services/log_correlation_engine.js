...
export class LogCorrelationEngine {
     ...
     */
    async generateSpearphishingCorrelation(config) {
        const { hostName = faker.internet.domainName(), userName = faker.internet.username(), alertTimestamp = new Date().toISOString(), logCount = 8, } = config;
        ...
        logs.push({
            ...
            'registry.data.strings': [
                `C:\\Users\\${userName}\\AppData\\Local\\Temp\\winupdate.exe`,
            ],
            'host.name': hostName,
            'process.name': 'invoice_2025.pdf.exe',
            'process.pid': faker.number.int({ min: 8000, max: 15000 }),
     ...
     */
    async generateCommandScriptingCorrelation(config) {
        const { hostName = faker.internet.domainName(), userName = faker.internet.username(), alertTimestamp = new Date().toISOString(), logCount = 6, } = config;
        const baseTime = new Date(alertTimestamp);
        const logs = [];
     ...
     */
    async generateProcessInjectionCorrelation(config) {
        const { hostName = faker.internet.domainName(), userName = faker.internet.username(), alertTimestamp = new Date().toISOString(), logCount = 5, } = config;
        const baseTime = new Date(alertTimestamp);
        const logs = [];
     ...
     */
    async generateCorrelatedLogs(alert, config = {}) {
        // Extract MITRE technique from alert if available
        const alertAny = alert;
     ...
     */
    async generateAttackScenario(alert, config = {}) {
