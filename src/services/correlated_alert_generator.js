...
export class CorrelatedAlertGenerator {
     ...
     */
    async generateCorrelatedAlert(config = {}) {
        const { hostName = faker.internet.domainName(), userName = faker.internet.username(), space = 'default', timestampConfig, logVolumeMultiplier = 6, useAI = false, useMitre = false, } = config;
        let alert;
     ...
     */
    async generateCorrelatedAlertBatch(alertCount, config = {}) {
        const { hostName = faker.internet.domainName(), userName = faker.internet.username(), logVolumeMultiplier = 6, } = config;
        const scenarios = [];
     ...
     */
    async generateAttackCampaign(alertCount, targetHosts, targetUsers, config = {}) {
        const scenarios = [];
     ...
     */
    extractLogsForIndexing(scenarios) {
        const alerts = [];
        const logs = [];
