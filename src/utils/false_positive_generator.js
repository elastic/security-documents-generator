 ...
 */
export function markAlertAsFalsePositive(alert, category) {
    // Select a random category if not provided
    const selectedCategory = category ||
        faker.helpers.arrayElement(Object.keys(FALSE_POSITIVE_CATEGORIES));
    ...
    const resolutionTime = new Date(originalTimestamp.getTime() +
        faker.number.int({ min: 300000, max: 7200000 }));
    return {
        ...alert,
        ...
        'kibana.alert.false_positive.resolution_time_minutes': Math.round((resolutionTime.getTime() - originalTimestamp.getTime()) / 60000),
 ...
 */
export function shouldMarkAsFalsePositive(falsePositiveRate) {
    return faker.number.float() < falsePositiveRate;
}
/**
 * Applies false positive logic to an array of alerts
 */
export function applyFalsePositiveLogic(alerts, falsePositiveRate) {
 ...
 */
export function generateFalsePositiveStats(alerts) {
