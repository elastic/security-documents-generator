...
import { MULTI_FIELD_TEMPLATES, getFieldCategories, getFieldsInCategory, getTotalFieldCount } from './multi_field_templates';
/**
...
export class MultiFieldGenerator {
     ...
     */
    generateFields(baseLog = {}, context = {}) {
     ...
     */
    analyzeLogContext(baseLog, context) {
     ...
     */
    adjustWeightForContext(fieldName, baseWeight, context) {
     ...
     */
    weightedFieldSelection(weightedFields, count) {
     ...
     */
    generateFieldValues(selectedFields, baseLog, context) {
        const fields = {};
     ...
     */
    applyFieldCorrelations(fields, context) {
        ...
        if (fields['network.analytics.malicious_ip_connections'] && fields['network.analytics.suspicious_domain_count']) {
            ...
            if (maliciousConnections > 0) {
                ...
                fields['network.analytics.suspicious_domain_count'] = Math.max(fields['network.analytics.suspicious_domain_count'], maliciousConnections + faker.number.int({ min: 0, max: 5 }));
                correlationsApplied++;
            }
        }
    ...
    detectAttackIndicators(baseLog) {
    ...
    determineSeverity(baseLog) {
        ...
        if (baseLog['event.severity'] || baseLog['log.level']) {
            ...
            if (['critical', 'error', 'high'].includes(level))
                return 'high';
            if (['warning', 'warn', 'medium'].includes(level))
                return 'medium';
            if (['info', 'debug', 'low'].includes(level))
                return 'low';
        }
    ...
    determineHostPerformance(baseLog) {
     ...
     */
    static generateQuick(baseLog = {}, fieldCount = 200, context = {}) {
 ...
 */
export function generateMultiFields(baseLog = {}, config = {}) {
