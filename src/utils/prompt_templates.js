...
export const generateAlertSystemPrompt = (context) => {
    const { hostName, userName, space, alertType = 'general', schemaExcerpt = '', } = context;
...
export const generateMitreAlertSystemPrompt = (context) => {
    const { hostName, userName, space, mitreContext = '', schemaExcerpt = '', attackChain = false, chainSeverity = 'medium', } = context;
...
export const generateAlertUserPrompt = (context) => {
    ...
    return `Generate a realistic security alert for host "${hostName}" and user "${userName}".${examples ? ` Reference examples: ${examples}` : ''}`;
};
export const generateMitreAlertUserPrompt = (context) => {
    const { hostName, userName, attackChain = false } = context;
    return `Generate a realistic security alert for host "${hostName}" and user "${userName}" based on the MITRE ATT&CK ${attackChain ? 'attack chain' : 'techniques'} provided.`;
};
export const generateEventUserPrompt = () => {
...
export const generateBatchAlertUserPrompt = (context) => {
    ...
    return `Generate exactly ${batchSize} realistic security alerts for these entities: ${JSON.stringify(entities)}.
...
export const TIMESTAMP_VALIDATION_RULE = 'CRITICAL: Use only dates from the past 30 days. NO future dates. NO 2023 or older dates.';
...
export const getFieldPrioritiesForAlertType = (alertType) => {
