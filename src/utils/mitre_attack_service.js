...
export const loadMitreData = () => {
    ...
    try {
        const filePath = path.resolve(process.cwd(), 'src/mappings', 'mitre_attack.json');
        const content = readFileSync(filePath, 'utf8');
        mitreDataCache = JSON.parse(content);
        cacheTimestamp = now;
        return mitreDataCache;
    }
    catch (error) {
...
export const getTechniquesForTactic = (mitreData, tacticId) => {
...
export const generateAttackChain = (mitreData, maxLength = 3) => {
    ...
    const availableInitialTechniques = mitreData.tactics[initialTactic]?.techniques || [];
    if (availableInitialTechniques.length === 0) {
        return null;
    }
    let currentTechnique = availableInitialTechniques[Math.floor(Math.random() * availableInitialTechniques.length)];
    // Build the chain
    for (let i = 0; i < maxLength; i++) {
        ...
        if (config.mitre?.includeSubTechniques &&
            ...
            subTechnique =
                techniqueData.subTechniques[Math.floor(Math.random() * techniqueData.subTechniques.length)];
        }
        ...
        if (techniqueData.chainNext && techniqueData.chainNext.length > 0) {
            const nextOptions = techniqueData.chainNext.filter((next) => !usedTechniques.has(next) && mitreData.techniques[next]);
    ...
    const hasCritical = chain.some((item) => criticalTechniques.includes(item.technique));
...
export const selectMitreTechniques = (mitreData, maxTechniques = 2) => {
    const config = getConfig();
    // Check if focus tactic is specified
    const focusTactic = config.mitre
        ?.focusTactic;
    let enabledTactics;
    if (focusTactic) {
    ...
    const availableTactics = enabledTactics.filter((tacticId) => mitreData.tactics[tacticId]);
    ...
    for (let i = 0; i < numTechniques; i++) {
        ...
        if (tactic.techniques.length > 0) {
            const randomTechniqueId = tactic.techniques[Math.floor(Math.random() * tactic.techniques.length)];
            // Select sub-technique if enabled
            let subTechnique;
            if (config.mitre?.includeSubTechniques) {
                const techniqueData = mitreData.techniques[randomTechniqueId];
                if (techniqueData?.subTechniques &&
                    techniqueData.subTechniques.length > 0) {
                    subTechnique =
                        techniqueData.subTechniques[Math.floor(Math.random() * techniqueData.subTechniques.length)];
                }
...
export const createMitreContext = (selectedTechniques, mitreData, attackChain) => {
...
export const calculateMitreRiskScore = (techniques, attackChain) => {
    if (attackChain) {
    ...
    const hasDangerousTechnique = techniqueIds.some((id) => dangerousTechniques.includes(id));
...
export const generateMitreFields = (selectedTechniques, mitreData, attackChain) => {
    const mitreFields = {};
    ...
    if (selectedTechniques.some((t) => t.subTechnique)) {
        const allTechniqueIds = selectedTechniques.map((t) => t.subTechnique ? t.subTechnique : t.technique);
        ...
        mitreFields['threat.technique.name'] = techniqueIds.map((id) => mitreData.techniques[id]?.name || id);
    }
    mitreFields['threat.tactic.id'] = tacticIds;
    mitreFields['threat.tactic.name'] = tacticIds.map((id) => mitreData.tactics[id]?.name || id);
    ...
    const { severity, riskScore } = calculateMitreRiskScore(selectedTechniques, attackChain);
