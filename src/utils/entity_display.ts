...
export function displayGeneratedEntities(
  ...
  if (entities.userNames && entities.userNames.length > 0) {
    ...
    const userDisplay =
      displayUsers.join(', ') + (moreCount > 0 ? ` and ${moreCount} more` : '');
  ...
  if (entities.hostNames && entities.hostNames.length > 0) {
    ...
    const hostDisplay =
      displayHosts.join(', ') + (moreCount > 0 ? ` and ${moreCount} more` : '');
  ...
  if (
    showKQLQueries &&
    (entities.userNames?.length || entities.hostNames?.length)
  ) {
    console.log('\n🔍 Useful KQL Queries:');
    ...
    if (entities.userNames && entities.userNames.length > 0) {
      ...
      if (entities.userNames.length > 0) {
        console.log(
          `  Filter by specific user: user.name: "${entities.userNames[0]}"`,
        );
      }
    }
    ...
    if (entities.hostNames && entities.hostNames.length > 0) {
      ...
      if (entities.hostNames.length > 0) {
        console.log(
          `  Filter by specific host: host.name: "${entities.hostNames[0]}"`,
        );
      }
    }
  }
  ...
  if (
    ...
    if (entities.userNames && entities.userNames.length > 0) {
      console.log(
        `  View data for user: user.name: "${entities.userNames[0]}"`,
      );
    }

    if (entities.hostNames && entities.hostNames.length > 0) {
      console.log(
        `  View data for host: host.name: "${entities.hostNames[0]}"`,
      );
    }

    if (entities.userNames && entities.hostNames) {
      console.log(
        `  Combined filter: user.name: "${entities.userNames[0]}" AND host.name: "${entities.hostNames[0]}"`,
      );
    }
  }
