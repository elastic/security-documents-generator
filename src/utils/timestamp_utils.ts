...
export interface TimestampConfig {
  ...
  pattern?:
    ...
    | 'attack_simulation'
    | 'weekend_heavy';
  eventDateOffsetHours?: number; // Legacy support
}
 ...
 */
export const getTimeRange = (config?: TimestampConfig): TimeRange => {
  ...
  if (!timestampConfig.startDate && !timestampConfig.endDate) {
    const offsetHours =
      timestampConfig.eventDateOffsetHours ||
 ...
 */
export const generateTimestamp = (config?: TimestampConfig): string => {
 ...
 */
const generateUniformTimestamp = (
  start: moment.Moment,
  end: moment.Moment,
): moment.Moment => {
 ...
 */
const generateBusinessHoursTimestamp = (
  start: moment.Moment,
  end: moment.Moment,
): moment.Moment => {
 ...
 */
const generateAttackSimulationTimestamp = (
  start: moment.Moment,
  end: moment.Moment,
): moment.Moment => {
 ...
 */
const generateWeekendHeavyTimestamp = (
  start: moment.Moment,
  end: moment.Moment,
): moment.Moment => {
 ...
 */
const generateWeekendTimestamp = (
  start: moment.Moment,
  end: moment.Moment,
): moment.Moment => {
  let attempts = 0;
 ...
 */
export const generateAttackChainTimestamps = (
  count: number,
  config?: TimestampConfig & {
    maxGapMinutes?: number;
    sequencePattern?: 'tight' | 'spread' | 'burst';
  },
): string[] => {
  const { start, end } = getTimeRange(config);
 ...
 */
export const getTimeRangeStats = (
  config?: TimestampConfig,
): {
  totalDays: number;
  totalHours: number;
