...
`,
    insider_threat_after_hours: (context) => `
...
`,
    supply_chain_compromise: (context) => `
...
export class EnhancedPromptGenerator {
     ...
     */
    static generatePrompt(alertType, baseContext, enhancementLevel = 'advanced') {
        const templateKey = this.mapAlertTypeToTemplate(alertType);
        const template = AI_PROMPT_TEMPLATES[templateKey];
        if (!template) {
        ...
        return this.addEnhancementDirectives(basePrompt, enhancementLevel, enrichedContext);
    }
    /**
     * Maps alert types to appropriate prompt templates
     */
    static mapAlertTypeToTemplate(alertType) {
        const typeMapping = {
     ...
     */
    static enrichContext(baseContext, enhancementLevel) {
        ...
        if (enhancementLevel === 'expert') {
            ...
            if (enriched.technique) {
                enriched.threat_actor = this.inferThreatActorFromTechnique(enriched.technique);
            }
        }
        return enriched;
    }
     ...
     */
    static addEnhancementDirectives(basePrompt, enhancementLevel, context) {
...
- Add supply chain and third-party risk indicators`;
     ...
     */
    static generateFallbackPrompt(alertType, context) {
        return `
...
Generate comprehensive security alert with all relevant fields populated.`;
     ...
     */
    static inferThreatActorFromTechnique(technique) {
        const techniqueActorMap = {
            'T1566.001': 'APT1',
            T1190: 'Lazarus Group',
