...
export async function setupSecurityMappings(): Promise<void> {
  ...
  console.log(
    '🔍 Note: For existing indices, you may need to reindex data to see new field mappings',
  );
  ...
  try {
    ...
    const securityIndices = indices.filter(
      (idx: any) =>
        idx.index &&
    ...
    console.log(
      `📊 Found ${securityIndices.length} security-related indices to update`,
    );

    // Create the mapping once
    const mapping = generateElasticsearchMapping(
      securityFields,
      'security-fields',
    );
    ...
    for (const idx of securityIndices) {
      try {
        ...
        if (error.meta?.body?.error?.type === 'illegal_argument_exception') {
          console.log(
            `  ⚠️  Skipped ${idx.index} (mapping conflict - this is normal)`,
          );
        } else {
    ...
    console.log(
      '  1. Refresh the field list in Kibana (Stack Management → Index Patterns)',
    );
    console.log(
      '  2. For best results, reindex existing data or wait for new data',
    );
  } catch (error) {
