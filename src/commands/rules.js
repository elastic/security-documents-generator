...
const generateNonOverlappingGapEvents = (ruleId, ruleName, fromHours, gapCount) => {
...
const ingestGapEvents = async (gapEvents) => {
    const client = getEsClient();
    if (!client)
        throw new Error('Failed to get ES client');
...
export const generateRulesAndAlerts = async (ruleCount, eventCount, options) => {
    // Create rules through Kibana API
    const ruleResults = await Promise.all(Array.from({ length: ruleCount }, () => {
    ...
    if (options.gapsPerRule > 0) {
        ...
        gapEvents = ruleResults.flatMap((rule) => {
            return generateNonOverlappingGapEvents(rule.id, rule.name || 'Unknown Rule', options.from, options.gapsPerRule);
        });
    }
...
export const deleteAllRules = async (space) => {
