...
import {
  MLDataGenerator,
  MLDataGenerationOptions,
} from '../ml/ml_data_generator';
 ...
 */
export async function generateMLData(
  params: MLGenerationParams,
): Promise<void> {
  ...
  try {
    ...
    for (const result of results) {
      ...
      console.log(
        `${status} ${result.jobId}: ${result.documentsGenerated} docs, ${result.anomaliesGenerated} anomalies`,
      );
    ...
    console.log(
      `  📈 Anomaly rate: ${((totalAnomalies / totalDocs) * 100).toFixed(3)}%`,
    );
    ...
    if (params.enableJobs) {
      ...
      console.log(
        '   You can view them in Kibana → Machine Learning → Anomaly Detection',
      );
    }
  } catch (error) {
 ...
 */
export async function generateMLDataForModules(
  params: MLGenerationParams,
): Promise<void> {
  const generator = new MLDataGenerator();
  ...
  const invalidModules = params.modules.filter(
    (m) => !validModules.includes(m),
  );
  ...
  try {
    const results = await generator.generateMLDataForModules(
      params.modules,
      options,
    );
    ...
    for (const moduleName of params.modules) {
      ...
      console.log(
        `  📊 Module Total: ${moduleSuccess}/${module.jobs.length} jobs, ${moduleDocs} docs, ${moduleAnomalies} anomalies`,
      );
    }
    ...
    const totalDocs = results.reduce(
      (sum, r) => sum + (r.success ? r.documentsGenerated : 0),
      0,
    );
    const totalAnomalies = results.reduce(
      (sum, r) => sum + (r.success ? r.anomaliesGenerated : 0),
      0,
    );
...
export function listMLJobs(): void {
  ...
  console.log(
    '  yarn start generate-ml-data --jobs auth_rare_user,linux_rare_sudo_user',
  );
  console.log('');
