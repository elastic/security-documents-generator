...
import { createRule, getAllRules, deleteRule, bulkDeleteRules } from '../utils/kibana_api';
...
const generateEvent = (from: number): Event => ({
  '@timestamp': moment().subtract(faker.number.int({ min: 1, max: from }), 'h').toISOString(),
  ...
  event: {
    category: faker.helpers.arrayElements(['authentication', 'process', 'network', 'file'], { min: 1, max: 2 }),
    type: faker.helpers.arrayElements(['start', 'end', 'info'], { min: 1, max: 2 }),
...
const generateNonOverlappingGapEvents = (
  ...
  const maxGapDuration = Math.max(minGapDuration + 1, Math.min(30, Math.floor(maxTimePerGap * 0.8))); // 80% of available time
  const maxSpaceBetweenGaps = Math.max(1, Math.floor(maxTimePerGap * 0.2)); // 20% of available time

  if (maxTimePerGap < 2) {
    console.warn(`Warning: Time window too small for ${gapCount} gaps. Each gap will be very short (${maxTimePerGap} minutes or less)`);
  }
  ...
  for (let i = 0; i < gapCount; i++) {
    const gapDuration = faker.number.int({ min: minGapDuration, max: maxGapDuration });
    const spaceBetweenGaps = faker.number.int({ min: 1, max: maxSpaceBetweenGaps });
...
console.log("gapEvents",gapEvents.length);
  await Promise.all([
    ingestEvents(events),
    ingestGapEvents(gapEvents)
  ]);
