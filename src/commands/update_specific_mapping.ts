 ...
 */
async function generateComprehensiveBehavioralMapping(): Promise<
...
> {
  ...
  console.log(
    `📊 Generated type mappings for ${Object.keys(fieldTypes).length} behavioral analytics fields`,
  );
  return fieldTypes;
}
 ...
 */
export async function updateSecurityAlertsMapping(
  indexName?: string,
): Promise<void> {
  const client = getEsClient();

  console.log(
    '🔧 Updating security alerts index mapping with comprehensive behavioral analytics fields...',
  );
  ...
  console.log(
    `🗺️  Generated mapping structure with ${Object.keys(properties).length} top-level properties`,
  );
  ...
  try {
    ...
    console.log(
      `📊 Added mappings for ${Object.keys(behavioralFields).length} behavioral analytics fields`,
    );
    console.log('');
    ...
    console.log(
      '  2. The fields should now appear as mapped instead of unmapped',
    );
    console.log(
      '  3. For best visualization, consider reindexing existing data',
    );
  } catch (error: any) {
    if (error.meta?.body?.error?.type === 'illegal_argument_exception') {
      console.log(
        '⚠️  Some field mappings could not be updated (this is normal for existing data)',
      );
