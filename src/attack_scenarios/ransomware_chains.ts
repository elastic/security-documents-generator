...
export interface RansomwareStage {
  name: string;
  tactic: string;
  techniques: string[];
  duration: {
...
export const RANSOMWARE_GROUPS: Record<string, RansomwareGroup> = {
  CONTI: {
    id: 'CONTI',
    name: 'Conti',
...
export const RANSOMWARE_CHAINS: Record<string, RansomwareChain> = {
  CONTI_ENTERPRISE: {
    id: 'CONTI_ENTERPRISE',
 ...
 */
export function generateRandomRansomwareChain(): RansomwareChain {
 ...
 */
export function getAllRansomwareChains(): RansomwareChain[] {
 ...
 */
export function getRansomwareChainsBySophistication(
  sophistication: 'low' | 'medium' | 'high' | 'expert',
): RansomwareChain[] {
  return Object.values(RANSOMWARE_CHAINS).filter(
    (chain) => chain.sophistication === sophistication,
  );
}
 ...
 */
export function getRansomwareChainsByIndustry(
  industry: string,
): RansomwareChain[] {
  return Object.values(RANSOMWARE_CHAINS).filter((chain) =>
    chain.target_profile.industries.includes(industry),
  );
}