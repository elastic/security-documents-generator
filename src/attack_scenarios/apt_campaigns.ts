...
export const THREAT_ACTORS: Record<string, ThreatActorProfile> = {
  APT1: {
    id: 'APT1',
    name: 'Comment Crew',
...
export const APT_CAMPAIGNS: Record<string, APTCampaign> = {
  OPERATION_AURORA: {
    id: 'OPERATION_AURORA',
 ...
 */
export function generateRandomAPTCampaign(): APTCampaign {
 ...
 */
export function getAllAPTCampaigns(): APTCampaign[] {
 ...
 */
export function getAPTCampaignsByComplexity(
  complexity: 'low' | 'medium' | 'high' | 'expert',
): APTCampaign[] {
  return Object.values(APT_CAMPAIGNS).filter(
    (campaign) => campaign.complexity === complexity,
  );
}
 ...
 */
export function getAPTCampaignsByActor(actorId: string): APTCampaign[] {
  return Object.values(APT_CAMPAIGNS).filter(
    (campaign) => campaign.threatActor.id === actorId,
  );
}